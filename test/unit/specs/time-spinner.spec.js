...
describe('TimePicker.vue', () => {
  ...
  it('should create a TimePicker component with hours, minutes and seconds', done => {
    ...
    vm.$nextTick(() => {
      const spiners = picker.$el.querySelectorAll('.lkui-time-picker-cells-list');
      expect(spiners.length).to.equal(3); // hh:mm:ss
      expect(spiners[0].querySelectorAll('.lkui-time-picker-cells-cell').length).to.equal(24);
      expect(spiners[1].querySelectorAll('.lkui-time-picker-cells-cell').length).to.equal(60);
      expect(spiners[2].querySelectorAll('.lkui-time-picker-cells-cell').length).to.equal(60);
      done();
    });
  });
  ...
  it('should create a TimePicker component with steps of 15 minutes', done => {
    ...
    vm.$nextTick(() => {
      const spiners = picker.$el.querySelectorAll('.lkui-time-picker-cells-list');
      const minutesList = [...spiners[1].querySelectorAll('.lkui-time-picker-cells-cell')];

      expect(spiners[0].querySelectorAll('.lkui-time-picker-cells-cell').length).to.equal(24);
      expect(minutesList.map(el => el.textContent).join(',')).to.equal('00,15,30,45');
      expect(spiners[1].querySelectorAll('.lkui-time-picker-cells-cell').length).to.equal(4);
      expect(spiners[2].querySelectorAll('.lkui-time-picker-cells-cell').length).to.equal(60);
      done();
    });
  });
