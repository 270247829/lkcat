...
describe('DatePicker.vue', () => {
  ...
  it('should create a DatePicker component and open the calendar with the current month', done => {
    ...
    picker.$el.querySelector('input.lkui-input').focus();
    vm.$nextTick(() => {
      const calendarBody = vm.$el.querySelector('.lkui-picker-panel-body .lkui-date-picker-cells:first-of-type');
      const calendarCells = [...calendarBody.querySelectorAll('.lkui-date-picker-cells-cell')].filter(el => {
        const prevMonth = el.classList.contains('lkui-date-picker-cells-cell-prev-month');
        const nextMonth = el.classList.contains('lkui-date-picker-cells-cell-next-month');
  ...
  it('should create a datetimerange component and pick 2 dates in the current month', done => {
    ...
    vm.$nextTick(() => {
      const displayField = vm.$el.querySelector('.lkui-input');
      const clickableCells = vm.$el.querySelectorAll('.lkui-date-picker-cells-cell');
      const lastMonthClass = 'lkui-date-picker-cells-cell-prev-month';
  ...
  it('should change type progamatically', done => {
    ...
    picker.handleIconClick();
    vm.$nextTick(() => {
      const panel = vm.$el.querySelector('.lkui-picker-panel-content');
      const dayPanel = panel.querySelector('[class="lkui-date-picker-cells"]');
      const monthPanel = panel.querySelector('.lkui-date-picker-cells-month');
      const yearPanel = panel.querySelector('.lkui-date-picker-cells-year');
  ...
  it('should render date-picker label correctly in zh-CN', done => {
    ...
    vm.$nextTick(() => {
      const now = new Date();
      const labels = vm.$el.querySelectorAll('.lkui-picker-panel-body .lkui-date-picker-header-label');
